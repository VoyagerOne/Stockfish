    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          && (!PvNode || ss->ply > 1 || thisThread->id() % 4 != 3))
      {
          ...
          if (!captureOrPromotion)
          {
              ...
              {
                  if (ss->statScore < 0 && thisThread->mainHistory[us][from_to(move)] < 0 && (*contHist[0])[movedPiece][to_sq(move)] < 0)
                      r++;
                  else if (ss->statScore > 0 && thisThread->mainHistory[us][from_to(move)] > 0 && (*contHist[0])[movedPiece][to_sq(move)] > 0)
                      r--;
              }
          }
